...
// ======================================================
const char* moduleId = "MOD-9477AB";
const int   slaveCapacity = 2;
const char* pairingSecret = "RMOomveYeoC2";   // stored securely
...
void onESPNowRecv(
  ...
  // --------------------------------------------------
  // SCAN REQUEST
  // Only respond if NOT paired OR if from paired master
  // --------------------------------------------------
  if (strcmp(msgType, "SCAN_REQ") == 0) {
    bool shouldRespond = false;

    if (!pairingState.isPaired) {
      // Not paired - respond to any scan
      shouldRespond = true;
      Serial.println("📡 Scan request received (unpaired mode)");
    } else {
      // Paired - only respond to paired master
      if (memcmp(info->src_addr, pairingState.masterMac, 6) == 0) {
        shouldRespond = true;
        Serial.println("📡 Scan request from paired master");
      } else {
        Serial.println("⚠️ Scan request from unknown device - ignoring");
      }
    }
