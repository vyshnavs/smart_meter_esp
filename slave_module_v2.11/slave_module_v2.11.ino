...
// ============================================================
//  FIRMWARE VERSION
//  __DATE__ and __TIME__ are injected by the compiler at the
//  moment of upload, producing a unique string every time.
//  This means checkFirmwareReset() will ALWAYS detect a new
//  flash and wipe all NVS data — no need to manually bump
//  a version number.
// ============================================================
const char* FIRMWARE_VERSION = "2.1.0";
const char* FIRMWARE_BUILD   = __DATE__ " " __TIME__;  // e.g. "Mar 30 2026 14:22:05"
...
//  EMON CALIBRATION
// ============================================================
const float VOLTAGE_CALIBRATION = 220.46;
const float CURRENT_CALIBRATION = 0.2974;
...
void IRAM_ATTR buttonISR() {
  ...
  if ((t - lastRelayCommandTime) < RELAY_COMMAND_LOCKOUT) return;
...
// ============================================================
//  NVS — FIRMWARE BUILD CHECK
//  Compares FIRMWARE_BUILD (compile timestamp) against the
//  value stored in NVS. Since the timestamp changes on every
//  upload, this always triggers a full NVS wipe on flash.
//  No need to manually bump any version string.
// ============================================================
void checkFirmwareReset() {
  preferences.begin("meta", false);
  String storedBuild = preferences.getString("build", "");
  preferences.end();

  if (storedBuild != FIRMWARE_BUILD) {
    Serial.println("[Boot] 🆕 New firmware upload — wiping ALL stored state");
    Serial.print  ("[Boot]    Old build: "); Serial.println(storedBuild);
    Serial.print  ("[Boot]    New build: "); Serial.println(FIRMWARE_BUILD);

    // Wipe every namespace — pairing, relay, and meta itself
    ...
    preferences.begin("meta",    false); preferences.clear(); preferences.end();

    // Write the new build timestamp into a fresh meta namespace
    preferences.begin("meta", false);
    preferences.putString("build", FIRMWARE_BUILD);
    preferences.end();

    // Reset runtime pairing state so module waits for a fresh pair
    ...
    relayState = false;  // safe default after wipe

    Serial.println("[Boot] ✅ ALL NVS cleared — module starts completely fresh");
  } else {
    Serial.println("[Boot] ✅ Same build — keeping stored state");
  }
}
...
Only two lines changed from your original:
```
VOLTAGE_CALIBRATION  234.26  →  220.46
CURRENT_CALIBRATION  2.28    →  0.2974