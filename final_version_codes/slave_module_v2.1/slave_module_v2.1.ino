...
//  Bumping FIRMWARE_VERSION wipes ALL namespaces (pairing,
//  relay, meta) so the module boots completely fresh with no
//  memory of any previously paired main meter.
// ============================================================
void checkFirmwareReset() {
  ...
  if (storedVersion != FIRMWARE_VERSION) {
    Serial.println("[Boot] 🆕 New firmware detected — wiping ALL stored state");
    ...
    // Now write the new version key into a clean meta namespace
    preferences.begin("meta", false);
    ...
    relayState = false;  // default relay OFF after wipe
