...
unsigned long lastRelayCommandTime = 0;
const unsigned long RELAY_COMMAND_LOCKOUT = 1000;
...
void loadPairingState() {
  ...
  if (pairingState.isPaired) {
    if (strcmp(pairingState.storedModuleId, moduleId) != 0 ||
      ...
      Serial.println("[Pairing] Module identity changed — auto-unpairing");
      clearPairingState();
      return;
    }
...
void clearPairingState() {
  ...
  Serial.println("[Pairing] Pairing state fully cleared from NVS");
}
...
void loadRelayState() {
  ...
  relayState = preferences.getBool("state", false);
  preferences.end();
...
void onESPNowRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
  ...
  if (strcmp(msgType, "SCAN_REQ") == 0) {
    ...
    Serial.print("[Scan] Response sent: "); Serial.println(r == ESP_OK ? "OK" : "FAIL");
  }
  ...
  // PAIR REQUEST
    ...
    if (moduleMatch && secretMatch) {
      if (pairingState.isPaired &&
        ...
        Serial.println("[Pair] Already paired with different master — rejected");
        ack.success = false;
      } else {
        ack.success = true;
  ...
  // UNPAIR COMMAND
    ...
    if (memcmp(info->src_addr, pairingState.masterMac, 6) != 0) {
      Serial.println("[Unpair] From unknown device — ignoring");
      return;
    }
  ...
  // RELAY CONTROL
    ...
    if (!pairingState.isPaired || memcmp(info->src_addr, pairingState.masterMac, 6) != 0) {
      Serial.println("[Relay] Not paired or unknown source — rejected");
      ack.success    = false;
