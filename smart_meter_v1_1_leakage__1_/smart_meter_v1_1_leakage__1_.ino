...
const float BURDEN_RESISTOR = 330.0; // In Ohms
const float OPAMP_GAIN = 41.0;       // Gain = 1 + (400k / 10k)
const float CT_RATIO = 1000.0;       // DTCT103C ratio is 1000:1
float CALIBRATION_FACTOR = 1.48000;  // Adjust per your calibration
const float NOISE_FILTER_MA = 1.0;   // Zero-noise filter threshold

// ============================================================
//  LEAKAGE TRIP THRESHOLD & STATE
// ============================================================
const float LEAKAGE_THRESHOLD_MA = 20.0;  // Trip relay if leakage >= this value (mA)
bool leakageTripped = false;               // true = relay was cut due to leakage
#define LEAKAGE_ALERT_INTERVAL 10000UL     // Throttle: re-publish alert every 10s while tripped
...
void publishLeakageAlert(float measuredMa) {
  publishFault(
    ...
    measuredMa,
    LEAKAGE_THRESHOLD_MA,
    "mA"
  );
  Serial.print("[Leakage] ⚠️ ALERT published | measured=");
  Serial.print(measuredMa, 2);
  Serial.print(" mA | threshold=");
  Serial.print(LEAKAGE_THRESHOLD_MA, 1);
  Serial.println(" mA");
}
...
void loop() {
  ...
  if (now - lastLeakageRead >= 500) {
    ...
    // Sample for 100ms (5 cycles at 50Hz)
    ...
    leakageCurrent_mA = primaryCurrent_A * 1000.0 * CALIBRATION_FACTOR;

    // Apply noise filter
    if (leakageCurrent_mA < NOISE_FILTER_MA) {
      leakageCurrent_mA = 0.0;
    }

    // ── LEAKAGE TRIP LOGIC ────────────────────────────────────
    // Trip relay if leakage exceeds threshold AND relay is currently on
    // NOTE: No auto-recovery — user must press BTN_RELAY to restore power
    if (leakageCurrent_mA >= LEAKAGE_THRESHOLD_MA && relayState && !relayLockedByServer) {
      ...
      Serial.print(leakageCurrent_mA, 2);
      Serial.println(" mA — relay OFF");
      publishRelayStatus();
    }

    // ── LEAKAGE ALERT (throttled, fires while tripped) ────────
  ...
  if (currentTamper) {
    ...
    if (lastTamperPin) {
      if (!relayLockedByServer && !leakageTripped) {
        // Only restore relay on tamper-clear if there is no active leakage trip
        relayState = true;
        digitalWrite(RELAY_PIN, LOW);
      }
      publishRelayStatus();
  ...
  // ── MANUAL RELAY BUTTON ───────────────────────────────────
  // Allows user to manually restore relay after a leakage trip.
  // Button is blocked if server-locked or tamper is active.
  // Pressing the button while leakageTripped clears the trip flag and turns relay ON.
  if (digitalRead(BTN_RELAY) == LOW && !tamperState && !relayLockedByServer) {
    if (leakageTripped) {
      // User is manually acknowledging and resetting the leakage trip
      leakageTripped = false;
      faultActive = false;
      relayState = true;
      digitalWrite(RELAY_PIN, LOW);
      saveRelayState();
      publishRelayStatus();
      Serial.println("[Leakage] 🔓 Trip cleared by user (button press) — relay ON");
    } else {
