...
// =====================================
// TAMPER TESTING CONFIGURATION
// =====================================
const unsigned long TAMPER_TEST_INTERVAL = 10000;
unsigned long lastTamperTest = 0;
int tamperTestCounter = 0;
...
// =====================================
// PUBLISH TAMPER ALERT (TESTING)
// =====================================
void publishTamperAlert() {
  tamperTestCounter++;
  
  int scenario = tamperTestCounter % 4;
  
  const char* tamperTypes[] = {
    "physical_breach",
    "seal_damage",
    "calibration_change",
    "unauthorized_access"
  ...
  const char* descriptions[] = {
    "Enclosure opened - unauthorized access detected",
    "Tamper seal has been damaged or removed",
    "Meter calibration values have been modified",
    "Unauthorized access attempt detected on meter"
  };
  ...
  doc["tamperType"] = tamperTypes[scenario];
  doc["severity"] = "critical";
  doc["description"] = descriptions[scenario];
  doc["detectionTime"] = "2025-03-15T12:05:34Z";
  
  char buffer[256];
  serializeJson(doc, buffer);
  
  if (client.publish(tamperTopic.c_str(), buffer)) {
    Serial.println("🔐 TEST TAMPER ALERT PUBLISHED:");
    Serial.print("  Type: ");
    Serial.println(tamperTypes[scenario]);
    Serial.print("  Severity: CRITICAL");
    Serial.println();
  } else {
...
void setup() {
  ...
  Serial.println("🔐 TAMPER TESTING MODE ENABLED");
...
void loop() {
  ...
  if (now - lastTamperTest >= TAMPER_TEST_INTERVAL) {
    publishTamperAlert();
    lastTamperTest = now;
  }
